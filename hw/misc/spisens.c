...
typedef struct SPISensor {
    ...
    uint8_t temperature;    // temperature as 7.1 fixed point
...
static void spi_sens_get_temperature(Object *obj, Visitor *v, const char *name,
                                   void *opaque, Error **errp)
{
    SPISensor *s = SPI_SENS(obj);
    uint16_t value = (s->temperature * 10) >> 1;

    visit_type_uint16(v, name, &value, errp);
}

/*
 * s->temperature is 7.1 fixed point and value that is received is in 0.1 degree.
 */
static void spi_sens_set_temperature(Object *obj, Visitor *v, const char *name,
                                   void *opaque, Error **errp)
{
    SPISensor *s = SPI_SENS(obj);
    uint16_t temp;

    if (!visit_type_uint16(v, name, &temp, errp)) {
        return;
    }
    if (temp > 1275) {
        error_setg(errp, "value %d.%01d C is out of range", temp / 10, temp % 10);
        return;
    }

    s->temperature = (uint8_t) ((temp << 1) / 10);
}
    ...
    if (!s->write_nread) {
        ...
        case 0:
            ...
            if (s->regs[REG_CTRL_OFFSET] & REG_CTRL_EN_MASK) {
                s->regs[REG_TEMPERATURE_OFFSET] = s->temperature;
            } else {
...
static const VMStateDescription vmstate_spi_sens = {
    ...
    .fields = (VMStateField[]){ VMSTATE_SSI_PERIPHERAL(parent_obj, SPISensor),
                                ...
                                VMSTATE_UINT8(temperature, SPISensor),
...
{
    object_property_add(OBJECT(d), "temperature", "uint16",
                        spi_sens_get_temperature,
                        spi_sens_set_temperature, NULL, NULL);
    return;
}

static void spi_sens_class_init(ObjectClass *klass, const void *data)
{
