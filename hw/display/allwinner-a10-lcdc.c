...
static void allwinner_a10_lcdc_tick(void *opaque)
{
    ...
    if (s->regs[REG_INDEX(REG_TCON_GINT0)] & (0x3 << 30)) {
        s->regs[REG_INDEX(REG_TCON_GINT0)] |= (0x3 << 14);
...
static void draw_line(void *opaque, uint8_t *d, const uint8_t *src, int width, int deststep)
{
    uint32_t data;
    unsigned int r, g, b;
    while (width > 0) {
...
static void allwinner_a10_lcdc_update_display(void *opaque)
{
    ...
    hwaddr frame_base = 0x4a100000;

    if (!is_enabled) {
        return;
    }

    step = LCD_WIDTH * 4;  // step = width * bpp >> 3;
    width = LCD_WIDTH;
    height = LCD_HEIGHT;
    ...
    if (s->invalidate) {
        framebuffer_update_memory_section(&s->fbsection,
                                          sysbus_address_space(SYS_BUS_DEVICE(s)), frame_base,
                                          height, step);
    }
...
static void allwinner_a10_lcdc_realize(DeviceState *dev, Error **errp)
{
    ...
    //address_space_init(&s->dma_as, s->dma_mr, "lcdc-dma");
}
...
/*
static Property allwinner_lcdc_properties[] = {
    DEFINE_PROP_LINK("dma-memory", AwLcdcState, dma_mr,
...
static void allwinner_a10_lcdc_class_init(ObjectClass *klass, void *data)
{
 ...
 //   device_class_set_props(dc, allwinner_lcdc_properties);
}
