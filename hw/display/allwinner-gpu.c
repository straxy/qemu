                                   ...
                                   uint64_t val, unsigned size)
{
    ...
    case REG_MALI_GP_CMD:
        if (val == 0x400) {
            s->regs[REG_INDEX(REG_MALI_GP_INT_RAWSTAT)] |= 1 << 19;
        }
        break;
    case REG_MALI_GP_MMU_COMMAND:
        if (val == 0) {
            s->regs[REG_INDEX(REG_MALI_GP_MMU_STATUS)] |= 0x1;
        } else if (val == 6) {
    ...
    case REG_MALI_PP0_MMU_COMMAND:
        if (val == 0) {
            s->regs[REG_INDEX(REG_MALI_PP0_MMU_STATUS)] |= 0x1;
        } else if (val == 6) {
    ...
    case REG_MALI_PP0_CTRL:
        if (val == 0x80) {
            s->regs[REG_INDEX(REG_MALI_PP0_INT_RAWSTAT)] = 1 << 12;
        }
        break;
    case 0xF0B8 ... AW_GPU_IOSIZE:
        qemu_log_mask(LOG_GUEST_ERROR, "%s: out-of-bounds offset 0x%04x\n",
